...
这页是 `styio`、`Spio`、`Vityo` 的跨仓协作流程总览。许可证、依赖商业风险和 secret 扫描的统一规则见 [审计、许可证与依赖合规](../standards/audit-license-and-dependency-policy.md)。
...
| `Spio` | 包管理、resolver、registry、publish、toolchain、`styio-protocol` 消费 | 工具链与包生态消费方 |
| `Vityo` | IDE / 运行视窗、工作区 UI、执行路由、hosted/local adapter | 产品交互与执行可视化消费方 |
...
2. `Spio` 和 `Vityo` 只能消费公开接口，不能反向定义语言语义。
...
`styio-protocol` 是 `styio` 对工具链、包管理器、IDE 和扩展公开的静态协议 / 交接契约，不是运行时套件，也不表示 `Spio`、`Vityo` 或扩展可以链接 `styio` 私有模块。
...
2. `Spio` 的 registry / resolver / publish 私有实现。
3. `Vityo` 的 Flutter UI 状态、主题系统或产品交互实现。

所有跨仓接口变更都应该先判断是否属于 `styio-protocol`。如果属于，必须在 `styio` 冻结协议形状，并由消费仓通过 contract / adapter tests 验证；如果只是单仓实现细节，就不要把它升级成跨仓协议。

## `styio` 到 `Vityo` 的语法上游关系

当 `styio` 添加 token、关键字、表达式、语句、资源语法、类型语法或诊断形状时，`styio` 是 `Vityo` 的上游。`Vityo` 不重新定义语法，只消费 `styio` 已冻结的语法事实和公共接口。
...
5. 如果 `styio` 暂未提供稳定接口，先向 `styio` 提出接口需求，不在 `Vityo` 内复制一套语法判定。

闭环标准：`styio` 的语法 gate 通过，并且 `Vityo` 的 IDE 相关测试能证明新语法在编辑、提示、诊断和运行入口上可用。

## `Vityo` 到 `styio` IDE 维护组件的需求上游关系

当 `Vityo` 在实现前端编辑器、运行视窗、补全、高亮、outline、diagnostic navigation 或 AI 辅助时，发现 `styio` 的基础 IDE 套件缺少必要的语法分析、增量解析、range、semantic token、completion source、hover source 或 diagnostic API，`Vityo` 是 `styio` IDE 维护组件的需求上游。

这条上游关系只适用于 IDE 能力需求，不改变语言语义的归属：语言语义仍由 `styio` 定义，`Vityo` 负责提出前端可用性和交互需求。
...
1. 在 `styio` 主仓内优化基础 IDE / LSP / workspace / parser service，而不是要求 `Vityo` 读取私有编译器结构。
...
4. 更新 `styio` 的 IDE/LSP/security/docs gate，并为 `Vityo` 提供消费说明。

闭环标准：`Vityo` 的前端需求有可复现样例，`styio` 的基础 IDE 套件提供稳定接口并通过本仓 gate，`Vityo` 再消费该接口并通过对应 Flutter / editor 测试。

## `styio` 到 `Spio` 的版本发布上游关系

当 `styio` 发布新版本时，`styio` 是 `Spio` 的上游。`Spio` 不决定编译器版本事实，但必须把新版本纳入包管理、工具链托管和生态通知流程。
...
4. 说明 `Spio` 需要更新的托管仓库、toolchain manifest、registry 元数据或 `styio-protocol` contract 文件。
...
6. 如果新版本包含破坏性接口变化，在 `Spio` 内保留兼容窗口、迁移诊断和回滚路径。

闭环标准：`styio` 版本发布门禁通过，并且 `Spio` 能通过托管版本发现、安装 / 切换 / pin、兼容矩阵、registry/publish 和通知相关验证。

### compile-plan v1 联动闭环

当 `styio` 广告 `supported_contracts.compile_plan:[1]` 时，`Spio` 可以把 compile-plan v1 视为已 live baseline，但必须同时满足三件事：
...
| `Spio` | governance / registry docs、resolver / registry / process tests、interop gates |
| `Vityo` | contracts / design docs、Flutter tests、prototype / hosted / execution adapter tests |
...
| 变更类型 | 先改 | 再改 | 必跑验证 |
| --- | --- | --- | --- |
| 新 token / 新语法 / 语法诊断 | `styio` | `Vityo` 高亮、补全、hover、diagnostic range；`Spio` 仅在 workflow 或 contract 受影响时跟进 | `styio` parser shadow/pipeline/security + `Vityo` editor/highlight/completion tests |
| 前端 IDE 需要基础语法分析能力 | `Vityo` 提需求 | `styio` IDE/LSP/workspace/parser service 补公共接口，`Vityo` 再消费 | `styio` IDE/LSP/security/docs gates + `Vityo` Flutter/editor tests |
| `styio` 新版本发布 | `styio` | `Spio` 版本托管仓库、toolchain index、compat matrix、registry/publish 元数据、通知消息 | `styio` release gates + `Spio` tool lifecycle/compat/registry/publish gates |
| 语言语义 / AST / IR | `styio` | `Spio`、`Vityo` 仅在接口受影响时跟进 | `styio` pipeline/security/parser shadow |
| `styio-protocol` / CLI / machine-info | `styio` | `Spio` contract、`Vityo` adapter | CLI diagnostics + consumer contract tests |
| `styio-protocol` / compile-plan | `styio` | `Spio` workflow、`Vityo` execution route | compile-plan tests + spio/view adapter tests |
| registry / publish | `Spio` | `styio` 仅在 `styio-protocol` 变化时跟进，`Vityo` 仅消费状态 | registry interop + resolver/process tests |
| IDE execution route | `Vityo` | `Spio` / `styio` 只在接口不足时补公开能力 | Flutter execution/hosted/local tests |
...
2. `Spio`：native tests、Python unit tests、interop gates、contract gates。
3. `Vityo`：Flutter unit tests、prototype tests、hosted/local adapter tests、CI lane。
...
| `Spio` | `docs/governance/` 或 `docs/planning/` |
| `Vityo` | `docs/design/` 或 `docs/plans/` |
...
2. `Spio`：强调 contract 消费、版本托管、发布通知、resolver/registry/process lifecycle、interop gates。
3. `Vityo`：强调 UI/adapter/runtime state machine、IDE service 需求、Flutter tests、hosted/local execution route。
