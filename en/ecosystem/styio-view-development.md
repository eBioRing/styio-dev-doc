...
这页只写 `styio-view` 的开发流程和规范。

`styio-view` 不是主编译器仓，而是 Styio 的专属 IDE、运行视窗和 AI 协作前端。

如果你要维护的是 VS Code 或别的宿主编辑器 Extension，不要走这页，先看 [Extensions 开发指引](extensions-development.md)。

## 当前阶段

按 2026-04-12 本地核对，`styio-view` 现在仍处于：

- `architecture bootstrap`
- docs-first
- 先冻结产品规格、系统架构、ADR、计划和平台边界，再进入实现

这意味着维护 `styio-view` 时，默认先动 `docs/`，不是先开 UI 代码。
...
- 产品规格：`docs/design/Styio-View-Product-Spec.md`
- 系统架构：`docs/design/Styio-View-System-Architecture.md`
- 协作规范：`docs/specs/CONTRIBUTOR-AND-AGENT-SPEC.md`
- 实施计划：`docs/plans/Styio-View-Implementation-Plan.md`
- ADR 索引：`docs/adr/INDEX.md`

所以 `styio-view` 的开发顺序应该由这些文档驱动，而不是从 README 直接跳到实现。
...
- 不能把 `styio-view` 退化成纯 Web 壳或传统 IDE 皮肤
...
`Styio-View-System-Architecture.md` 当前的主线是：
...
这意味着 `styio-view` 和 `styio` 的正确关系不是“直接绑 C++ ABI”，而是：
...
因此 `styio-view` 的开发指引必须始终带着平台约束走，不能把桌面方案直接复制到移动端。
