...
test -f _book/zh/index.html
test -f _book/en/index.html

if [[ "$WITH_BROWSER" -eq 1 ]]; then
  rm -rf "$ARTIFACT_DIR"
  npx --yes playwright install chromium
  python3 -m http.server "$PORT" --directory "$ROOT/_book" >"$ROOT/.tmp-doc-server.log" 2>&1 &
  SERVER_PID=$!
  trap 'kill "$SERVER_PID" >/dev/null 2>&1 || true' EXIT
  sleep 2
  mkdir -p "$ARTIFACT_DIR"

  test -f _book/zh/interfaces/core-interfaces.html
  grep -q "Styio 维护者手册" _book/zh/index.html
