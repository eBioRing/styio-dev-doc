...
AI 不能为代码负责，因此也不能为代码签名。

- 禁止使用 AI / 机器人账号（例如 Cursor Agent、Claude Code / Claude、Copilot 及同类编程代理）作为 Git commit 的 **author** 或 **committer** 进入受管分支。
- 人类作者在使用 AI 生成或协助改动时，必须以自己的身份签名并提交；AI 最多可作为草稿来源，不得作为署名作者。
- 含有 AI / 机器人签名的 Pull Request 视为草稿：须由人类作者重签后再合并；不得直接合并进默认分支。
- 该规则适用于 Styio 生态受管仓库的协作，与具体使用哪一家 AI 工具无关。

详细组织宪法见 [AI 辅助与提交签名（组织宪法）](./ai-authorship-constitution.md)。本条对 SymPolicy 组织全部仓库生效。
